...
#include <vmouse_client/registry.hpp>
...
void Mouse::moveCursor(LONG x, LONG y) {
    ...
    while(!isAborted()) {
        ...
        if(ERROR_SUCCESS != getCurrentCursorPositionError) throw std::runtime_error{ "ERROR_CURSOR_PROCESSING" };

        LONG distance = static_cast<LONG>(sqrt(pow(x - currentPoint.x, 2) + pow(y - currentPoint.y, 2)));

        std::cout << "current x " << currentPoint.x << std::endl;
        std::cout << "current x " << currentPoint.y << std::endl;
        std::cout << "distance " << distance << std::endl;
        std::cout << "DISTANCE_TOLERANCE " << DISTANCE_TOLERANCE << std::endl;
        if(distance <= DISTANCE_TOLERANCE) {
            std::cout << "distance lower than tolerance" << std::endl;
            return;
        }
        ...
        std::cout << "Speed " << (int)xSpeed << " " << (int)ySpeed << std::endl;
